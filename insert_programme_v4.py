#!/usr/bin/env python3
"""
Script pour insérer les priorités programme et mesures dans Strapi via l'API
Version 4: Adaptation pour la nouvelle structure avec priorite_programme (oneToOne)
"""
...
try:
    ...
    # Supprimer les priorités programme
    response = make_request_with_retry(requests.get, f'{STRAPI_URL}/api/priorites-programme', headers=headers)
    if response.ok:
        priorites = response.json().get('data', [])
        for priorite in priorites:
            make_request_with_retry(requests.delete, f'{STRAPI_URL}/api/priorites-programme/{priorite["id"]}', headers=headers)
            time.sleep(0.1)
        print(f"✓ {len(priorites)} priorités programme supprimées")
except Exception as e:
    print(f"Erreur lors de la suppression: {e}")

# Données des priorités programme
priorites_data = [
    {
...
# Mesures par priorité (index commence à 0)
mesures_data = [
    # Priorité 1: redonner de l'air à notre ville
    [
    ...
    # Priorité 2: une ville sereine et sécurisée
    [
    ...
    # Priorité 3: préserver nos biens communs
    [
    ...
    # Priorité 4: solidarité et vie locale
    [
    ...
    # Priorité 5: Finances, économie, gestion et transparence
    [
...
# Créer les priorités programme
priorite_ids = []
print("\nCréation des priorités programme...")
for i, priorite_data in enumerate(priorites_data):
    try:
        print(f"Priorité {i+1}: {priorite_data['titre'][:50]}...")
        response = make_request_with_retry(
            requests.post,
            f'{STRAPI_URL}/api/priorites-programme',
            headers=headers,
            json={'data': priorite_data}
        )
        if response.ok:
            priorite_id = response.json()['data']['id']
            priorite_ids.append(priorite_id)
            print(f"  ✓ Créée (ID: {priorite_id})")
        else:
            print(f"  ✗ Erreur: {response.status_code} - {response.text[:100]}")
            priorite_ids.append(None)
        time.sleep(0.5)
    except Exception as e:
        print(f"  ✗ Exception: {e}")
        priorite_ids.append(None)

# Créer les mesures avec relation oneToOne
print("\nCréation des mesures...")
total_mesures = 0
for priorite_index, mesures_list in enumerate(mesures_data):
    priorite_id = priorite_ids[priorite_index]
    if priorite_id is None:
        print(f"Priorité {priorite_index+1}: ignorée (non créée)")
        continue
    
    print(f"\nPriorité {priorite_index+1} - {len(mesures_list)} mesures:")
    for ordre, mesure_titre in enumerate(mesures_list, 1):
        try:
            # Pour une relation oneToOne, on passe l'ID directement (pas de tableau)
            mesure_data = {
                'titre': mesure_titre,
                'ordre': ordre,
                'priorite_programme': priorite_id  # oneToOne: ID direct
            }
            response = make_request_with_retry(
                requests.post,
...
print(f"\n{'='*80}")
print(f"✅ Terminé: {len([p for p in priorite_ids if p])} priorités et {total_mesures} mesures créées")
print(f"{'='*80}")
