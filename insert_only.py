#!/usr/bin/env python3
"""Script pour insérer uniquement les priorités et mesures dans Strapi"""
...
for i, priorite_data in enumerate(priorites_data):
    try:
        print(f"Priorité {i+1}: {priorite_data['titre']}")
...
for priorite_index, mesures_list in enumerate(mesures_data):
    ...
    print(f"Priorité {priorite_index+1} - {len(mesures_list)} mesures:")
    for ordre, mesure_titre in enumerate(mesures_list, 1):
        try:
            mesure_data = {
                'titre': mesure_titre,
                'ordre': ordre,
                'priorite_programme': priorite_id
            }
            response = requests.post(
            ...
            if response.ok:
                ...
                print(f"  ✗ {ordre}. Erreur {response.status_code}")
            time.sleep(0.3)
        except Exception as e:
